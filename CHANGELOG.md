...
All notable changes to avocado-bsp-jetson-agx-orin-devkit are documented in this file.
...
### Changed
- **The extension is now `avocado-bsp-jetson-agx-orin`** (was `avocado-bsp-jetson-agx-orin-devkit`),
...
- **Publishes to both feeds under each feed's own target name.** The 2026 feed
  calls this target `jetson-agx-orin`, the 2024 feed calls it `jetson-agx-orin-devkit`;
  `supported_targets` lists both and each release/test matrix leg names the one
  its feed uses. Previously both legs published to `jetson-agx-orin-devkit`, which
  is not a target in the 2026 feed — so nothing ever landed in
  `2026/next/target/jetson-agx-orin-ext`.
...
### Added
- Initial release: Board support for the Nvidia Jetson AGX Orin devkit.
